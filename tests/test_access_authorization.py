...
async def test_unauthorized_request():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get("http://localhost:8000/pets/") as response:
...
async def test_authorized_request():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        #Add new user
        async with session.post(url="http://localhost:8000/users/", json=user_data) as response:
            assert response.status == HTTPStatus.OK 


        #Get access token
        async with session.post(url="http://localhost:8000/users/access/", json=user_data) as response:
            assert response.status == HTTPStatus.OK 
        ...
        async with session.get("http://localhost:8000/pets/", headers=valid_headers) as response:
            assert response.status == HTTPStatus.OK
